...
> Extended-mode runbook. New Lean projects follow the five-stage workflow in `SKILL.md` and `stage-lifecycle.md`. Nothing in this file adds a gate or required field to Lean mode.
...
The validator is the only mandatory command at every stage flip. Local execution stages may also use the persistent tools in [local-runtime-environment.md](./local-runtime-environment.md), especially `xingchen-transcribe\scripts\transcribe_faster_whisper.py` for recording-first ingest and the Remotion adapter harness for render. Adapter usage follows [plugin-adapter-policy.md](./plugin-adapter-policy.md); Codex plugins, Claude Code local skills, local CLIs, and manual implementation lanes execute approved routes and must write trace records, but do not replace state.
...
Recording-first execution: use the persistent faster-whisper helper documented in [local-runtime-environment.md](./local-runtime-environment.md) to create `transcript.tsv`, `recording-cleanup-report.md`, cleaned or accepted audio paths, `sources.transcript.segments`, and `sources.recording_correction`; do not install transcription dependencies into a temporary project folder. Then run or route to `xingchen-speech-rhythm` to create `sources.speech_rhythm` from SRT/transcript timing. Recording correction and rhythm analysis are mandatory before visual design unless `sources.speech_rhythm.status = "not_needed"` with a concrete reason.
...
State paths required non-empty: `script.spoken_script.blocks` (≥1), `script.beat_map.scenes` (≥1). Narration-led projects should also fill `script.speech_rhythm_plan` before Script Lock, unless it records `status: "not_needed"` with a concrete reason.
...
Owning skill: `xingchen-script-polish`, with `xingchen-speech-rhythm` supporting narration rhythm planning.
Language-game correction: before Script Lock, apply [language-game-correction.md](./language-game-correction.md) to key terms, metaphors, and style claims. If a term has no public use in narration, on-screen text, proof, or beat behavior, rewrite it or route the script back to editorial review.
Speech rhythm: before Script Lock on voice-led projects, run `xingchen-speech-rhythm` in plan mode. Use its risks to split long lines, slow professional terms, place breath points, and write `script.beat_map.scenes[].rhythm_cues` where helpful.
...
Director board: `xingchen-director-board` reads the user's material page by page / beat by beat, reads corrected recording rhythm and speech-rhythm handoff, binds material and narration to scenes, runs [visual-collaboration-contract.md](./visual-collaboration-contract.md), runs `superpowers/brainstorming` for each scene picture and scene-to-scene flow, designs each frame, chooses component/tech routes, and writes `visual.director_board` plus MD/JSON board artifacts. The visual collaboration pass should propose bold picture options first, then constrain them with recording timing, rhythm flags, 9:16 short-video readability, proof safety, subtitle safe regions, and anti-PPT rules.
Language-game correction: before Visual Lock, apply [language-game-correction.md](./language-game-correction.md). Vague aesthetic or metaphor words must become source-layer meaning, scene questions, selected-direction reasons, camera reveals, object behaviors, on-screen text purpose, or lookdev criteria. Otherwise keep the board at `manual_review_required` or `blocked`.
...
Optional Vox/Remotion editorial-motion influence: when selected, read [vox-remotion-visual-style.md](./vox-remotion-visual-style.md). `xingchen-art-direction` records `visual_style_influence` and `editorial_world_system`; `xingchen-director-board` expresses the scene contract through existing scene layers; `xingchen-visual-compiler` exposes the layer stack, Remotion-owned proof/text overlays, beat sync, and prop controls; `xingchen-lookdev` audits the style-specific checks. This is never a new renderer route or fixed template.
Adapter pass: after scene specs exist, use Codex plugin skills when available, external APIs only when actually available, or Claude Code local skills / local CLI / manual implementation lanes when not. HyperFrames outputs must write `render.hyperframes_candidates[]` plus `render.plugin_adapter_runs[]`; DOM/SVG-heavy HyperFrames scenes must produce a GSAP timeline candidate or record a concrete skip reason. Seedance-style manual generation starts as `render.ai_video_prompt_requests[]`, then the user-generated file is registered as `render.ai_video_candidates[]` plus `render.plugin_adapter_runs[]` and can only be promoted as a Remotion-controlled `video_plate`; Remotion implementation or promotion work must write `render.plugin_adapter_runs[]` and the affected `render.scene_motion_specs[]`, `render.jobs[]`, or render-pack paths.
HTML 3D route: when a scene board selects `html_3d`, apply [html-3d-scene-route.md](./html-3d-scene-route.md). The route must say what the 3D camera/depth reveals.
...
Owning skill: `xingchen-lookdev`. Audits run via [scene-composition-audit.md](../../xingchen-lookdev/references/scene-composition-audit.md) (consumes the frame_strategy / distortion_policy / anchor_region / proof_regions / subtitle_safe_region / z_order_plan fields written by visual-compiler upstream). If `visual_style_influence.source = "vox_remotion_visual_style"`, also run or record the style-specific checks from [vox-remotion-visual-style.md](./vox-remotion-visual-style.md); unsupported aesthetic checks push to `manual_review_required`.
...
Runtime doctor: for Remotion-bound project folders, run the static runtime doctor before treating preview files as lookdev evidence; run still-frame doctor checks during explicit retest or render debugging.
Next stage: `render`.
...
Next stage: `knowledge-writeback`.

## knowledge-writeback

State paths required non-empty: `review.knowledge_writeback.status` or a project-local `knowledge-writeback-candidates.md/json` referenced from review notes.
Artifacts on disk required: when candidates exist, a writeback candidate file lists accepted writes, rejected candidates, target vault paths or skill reference paths, and any asset registry updates.

Run [knowledge-writeback-pass.md](./knowledge-writeback-pass.md). Route reusable video methods to stevenmind video creation pages, technical contracts to skill references or `04 Wiki\技术栈`, and reusable assets to `C:\Users\liuzh\Videos\douyin\visual-assets\registry\asset-registry.json`.

Gate: none, but silent skip is invalid. If there is nothing to write, set `status = "not_needed"` with a concrete reason.
...
4. Validator passes: `node ../schema/validate-project-state.mjs path\to\project-state.json` exits 0.
